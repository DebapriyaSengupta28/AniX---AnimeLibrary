...
function TrendingPage() {
  ...
  useEffect(() => {
    // Fetch trending anime data from AniList API
    const fetchAnimeData = async () => {
      try {
        ...
        // Only update the trendingAnime state with data from the first page
        if (currentPage === 1) {
          setTrendingAnime(response.data.data.Page.media);
        } else {
          // Update trendingAnime state by merging new data with existing data
          setTrendingAnime(prevAnime => [...prevAnime, ...response.data.data.Page.media]);
        }
      } catch (error) {
  ...
  return (
    ...
    >
      <Navigation2 />
      <h1 style={{ marginBottom: "50px" }}>Trending</h1>
